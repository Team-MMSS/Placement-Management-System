...
def update():
    ...
    cur = mydb.cursor()
    cur.execute(
        "update student_table set Full_name = %s,Email_Id=%s,Phone_no=%s,Gender=%s,Branch=%s,Year=%s,CGPA=%s,HSC=%s,SSC=%s,CP_Profile=%s where  registration_number =%s ",
...
def search():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='Mudassir@2003', database='placement')
    cur = mydb.cursor()
    sql = 'select * from student_table where branch="%s"' % search_var.get()
    cur.execute(sql)
    result = cur.fetchone()
